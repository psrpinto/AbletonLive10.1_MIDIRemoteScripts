...
class OSCClient:
    ...
    def __init__(self, touchAble, udpClient=None, tcpServer = None, address=None, msg=None):
        """
        ...
        self.touchAble.log_message("OSCClient init")
    ...
    def send(self, address=None, msg=None, udp = 0):
        ...
        if self.udpClient is None:
            ...
            #self.touchAble.log_message("self.udpClient is None:")
            return
        ...
        if not address and not self.address:
            #self.touchAble.log_message("if not address and not self.address:")
            # SHOULD RAISE EXCEPTION
            return
        ...
        if not address:
            if not self.address:
                # SHOULD RAISE EXCEPTION
                #self.touchAble.log_message("if not self.address:")
                return
           ...
           oscMessage.append(msg)
            ...
            for m in msg:
                if type(m) not in (str,int,float):
                    #self.touchAble.log_message("if type(m) not in (str,int,float):")
                    # SHOULD RAISE EXCEPTION
                    return
                oscMessage.append(m)      
        elif msg == None:
        ...
        if udp: 
            ...
            self.tcpServer.send(oscMessage.getBinary())

        
class OSCServer:
        
    def __init__(self, touchAble = None, dst=None, dstPort=None, src=None, srcPort=None):
        
        self.touchAble = touchAble
        #self.touchAble.log_message("OSCServer logging from touchable?")
        ...
        self.tcpServer = TCPServer(self, src, srcPort)
        self.udpClient = UDPClient(dst, dstPort)
        self.udpClient.open()
        
        self.oscClient = OSCClient(self.touchAble, self.udpClient, self.tcpServer, None, None)
        
        # Create our callback manager and register some utility
        # callbacks to show how its done.
        
        self.callbackManager = OSC.CallbackManager(self)
        self.callbackManager.add(self.callbackEcho, '/remix/echo')
        self.callbackManager.add(self.changeIP, '/setip')
    ...
    def changeIP(self, msg=None):
        self.oscClient.send('/connected', 2)
    ...
    def sendOSC(self, address=None, msg=None):
        ...
        """
        #self.touchAble.log_message(address)
        #self.touchAble.log_message(msg)

        if address != None and msg != None:
            self.oscClient.send(address, msg)


    def sendOSCUDP(self, address=None, msg=None):
        """
        A convienence function so we don't have to dig into the objects
        every time we want to send an OSC packet.
        """
        ...
        if address != None and msg != None:
            self.oscClient.send(address, msg, 1)
    ...
    def processIncomingUDP(self):
        ...
        * The Ableton Live Python environment, although it includes the thread
        ...
        self.tcpServer.processIncomingUDP()
...
class UDPClient:
    ...
    def __init__(self, dst=None, dstPort=None):
        ...
        self.udpaddr = (self.dst, self.dstPort)
...
class TCPServer:
    ...
    def __init__(self, oscServer = None, src = None, srcPort = None):
        """
        ...
        self.oscServer.touchAble.log_message("TCPServer init")
    ...
    def acceptConnection(self):
        try:
            ...
            self.rxset.append(self.conn)
            self.connected = 1
            self.listening = 0
            self.accepting = 0
            self.oscServer.touchAble.log_message("TCPServer accepted connection")
            ...
            self.listening = 1
            self.accepting = 0
            self.oscServer.touchAble.error_logger("TCPServer accept exception falled")
    ...
    def processIncomingUDP(self):
        ...
        try:
            ...
            # program. But. As it turns out. It doesn't. //probably gets killed by Live
            

            #rxfds, txfds, exfds = select.select(self.rxset, self.txset, self.rxset)
            #for sock in rxfds:
                #if sock is self.TCPSock:
            if self.listening == 1 and self.connected == 0 and self.accepting == 0:
                self.accepting = 1
                thread.start_new_thread(self.acceptConnection, ())

            elif self.connected == 1 and self.conn != None:
                while 1:
                    try:
                        rem = 4

                        length_data = self.conn.recv(rem)
                        length_received = len(length_data)
                        rem = rem - length_received
                        while rem > 0:
                            rxfds, txfds, exfds = select.select(self.rxset, self.txset, self.rxset)
                            data2 = self.conn.recv(rem)
                            length_received = len(data2)
                            rem = rem - length_received
                            length_data.append(data2)

                        packet_length = OSC.readInt(length_data)[0]



                        rem = 4
                        type_data = self.conn.recv(rem)
                        length_received = len(type_data)
                        rem = rem - length_received
                        while rem > 0:
                            rxfds, txfds, exfds = select.select(self.rxset, self.txset, self.rxset)
                            data2 = self.conn.recv(rem)
                            length_received = len(data2)
                            rem = rem - length_received
                            type_data.append(data2)

                        type = OSC.readInt(type_data)[0]



                        rem = 4
                        tag_data = self.conn.recv(rem)
                        length_received = len(tag_data)
                        rem = rem - length_received
                        while rem > 0:
                            rxfds, txfds, exfds = select.select(self.rxset, self.txset, self.rxset)
                            data2 = self.conn.recv(rem)
                            length_received = len(data2)
                            rem = rem - length_received
                            tag_data.append(data2)

                        tag = OSC.readInt(tag_data)[0]

                        rem = packet_length
                        packet_data = self.conn.recv(rem)
                        length_received = len(packet_data)
                        rem = rem - length_received
                        while rem > 0:
                            rxfds, txfds, exfds = select.select(self.rxset, self.txset, self.rxset)
                            data2 = self.conn.recv(rem)
                            length_received = len(data2)
                            rem = rem - length_received
                            packet_data.append(data2)


                        if not packet_data:
                            self.oscServer.sendOSCUDP("/NSLOG_REPLACE", "NO PACKET DATA")
                            self.oscServer.touchAble.error_logger("NO PACKET DATA")
                            ...
                            if packet_data != '\n':
                                ...
                                if self.callbackManager:
                                    #self.oscServer.sendOSCUDP("/NSLOG_REPLACE", "self.srcPort")
                                    self.callbackManager.handle(packet_data)
                    except socket.error, e3:
                        err = e3.args[0]
                        if err == errno.EAGAIN or err == 10035:
                            return
                        else:
                            # a real error occured
                            print e3
                            print "Connection closed by remote end"
                            #self.oscServer.sendOSCUDP("/NSLOG_REPLACE", ("CONNETION CLOSED BY REMOTE", str(e3)))
                            self.oscServer.touchAble.error_logger("Connection closed by remote end")

                            self.rxset.remove(self.conn)
                            self.conn.close()
                            self.connected = 0
                            self.listening = 1
                            self.conn = None
    ...
    def bind(self):
        ...
        while True:
            try:
                self.TCPSock.bind((self.src,self.srcPort+port_add))
                self.TCPSock.listen(1)
            except:
                port_add += 1
                port_add = port_add % 10
                self.oscServer.touchAble.error_logger("TCPServer port in use - add 1 to portnumber")

            
            else:
                self.srcPort = self.srcPort + port_add
                print "listening on port", self.srcPort
                self.oscServer.sendOSCUDP("/script/port_changed", self.srcPort)
                self.listening = 1

                self.oscServer.touchAble.log_message("TCPServer is now listening")
                break
    ...
    def send(self, data):
        ...
        # Only send if we have data.
        if not data == '' and self.conn != None:
            data_to_send = '' + OSC.OSCArgument(length)[1] + OSC.OSCArgument(type)[1] + OSC.OSCArgument(tag)[1] + data
